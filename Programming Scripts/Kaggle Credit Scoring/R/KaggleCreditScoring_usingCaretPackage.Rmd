...
_(**note**: in more extreme cases where one class is much, much rarer than the other to the order of 1000 or 10,000 times, our model fitting procedures would need to be tweaked too; but this case is not so extreme)_
...
The proportions of missing data points per column are as follows:
...
```{r}
cs_train_mean_MonthlyIncome = mean(cs$MonthlyIncome, na.rm=TRUE)
cs_train_mean_NumberOfDependents = mean(cs$NumberOfDependents, na.rm=TRUE)
...
```{r}
small_p <- 1e-6
low_log_odds <- log(small_p / (1 - small_p))
high_log_odds <- log((1 - small_p) / small_p)
log_odds_thresholds <- seq(from=low_log_odds, to=high_log_odds, length.out=1000)
prob_thresholds <- exp(log_odds_thresholds)
...
i <- min(which(boost_oos_performance$sensitivity < sensitivity_threshold))
