...
cl <- makeCluster(detectCores() - 2)   # create a compute cluster using all CPU cores but 
...
rf_train <- train(medv ~ ., data=boston_housing,
...
rf_predict <- predict(rf_train$finalModel, newdata=boston_housing)
```

This Random Forest model has an estimated OOB RMSE of **`r formatC(min(rf_train$results$RMSE), format='f', digits=3)`**.


# Boosted Trees model

```{r message=FALSE}
boost_train <- train(medv ~ ., data=boston_housing,
...
boost_predict <- predict(boost_train$finalModel, newdata=boston_housing, n.trees=B)
```

This Boosted Trees model has an estimated OOS RMSE of **`r formatC(boost_train$results$RMSE, format='f', digits=3)`**.
...
varImpPlot(rf_train$finalModel, main="Random Forest's Variable Importance")
plot(summary(boost_train$finalModel, plotit=FALSE), main="Boosted Trees Model's Variable Importance")
```
