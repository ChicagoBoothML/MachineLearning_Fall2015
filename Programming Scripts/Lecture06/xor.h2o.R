...
dfh2o = as.h2o(data.frame(x=x,y=g))
...
phat = as.matrix( h2o.predict(model, dftest)[,3] )
phat = matrix(phat, length(px1), length(px2))
...
tmp.df = as.h2o(data.frame(x.1=c(-1, -1, 1, 1), x.2=c(-1, 1, -1, 1)))
...
phat = as.matrix( h2o.predict(model5, dftest)[,3] )
phat = matrix(phat, length(px1), length(px2))
...
phat = as.matrix( h2o.predict(model10, dftest)[,3] )
phat = matrix(phat, length(px1), length(px2))
...
phat = as.matrix( h2o.predict(model10.reg, dftest)[,3] )
phat = matrix(phat, length(px1), length(px2))
