---
title: "Homework 04: Orange's Customer Relationships: DATA PREPROCESSING Steps"
...
_**NOTE**: because some sections below take quite long to run, we've commented them out. The script will jump straight to the selected X variables._
...
It seems that **all of the remaining `r nb_input_features` strong features** are **numeric**! _(meaning the effort we spent on cleaning the categoricals has come to naught... but we couldn't have known that without trying)_
...
prepare_oos_input_features <- function(X_OOS) {
  ...
  # *** NOTE: ***
  # If you also have categorical features in the final Training dat
  # you'll need to do something to clean those categoricals in Validation and Test too;
  # here we don't need to do that because our selected features are all numeric
